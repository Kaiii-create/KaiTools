...
- 备选：`KTool_*_x64.msi`，适合需要 MSI 的场景。

安装器检测到 D 盘时，默认安装到 `D:\KTool`，也可以在安装界面手动选择其他目录；电脑没有 D 盘时会安全回退到 Windows 默认目录。

> 当前发布包暂未进行商业代码签名。Windows 可能显示“未知发布者”，请确认安装包来自本仓库的 Releases 页面后再继续安装。
...
仓库已配置 GitHub Actions。推送 `v*` 标签后，会在 Windows 构建机上自动生成 NSIS `.exe` 与 `.msi`，并上传到 GitHub Releases。
