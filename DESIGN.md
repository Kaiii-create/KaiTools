...
| `--ktool-surface-inset` | `#f3f4f6` | `#121419` | 输入/输出编辑区内陷底 |
...
| 右侧按钮数量 | ≤ 3（历史 / 主题 / 设置）；命令面板并入 Sidebar 搜索 |
...
- **输入区**：`--ktool-surface-inset` 内陷底（可编辑=下沉感）。
- **输出区（结果）**：`--ktool-surface` 亮底 + `--ktool-shadow-sm`（结果=浮现感），用明度差区分主次，而非两个相同卡片。
